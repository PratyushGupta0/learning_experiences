...
void DubinsTrajectory::loadParams(ros::NodeHandle& nh_private) {
    nh_private.getParam("x_way", end_pos_.x());
    nh_private.getParam("y_way", end_pos_.y());
    nh_private.getParam("z_way", end_pos_.z());
    nh_private.getParam("radius", radius);
    nh_private.getParam("centre_x", centre_x);
    nh_private.getParam("centre_y", centre_y);
    nh_private.getParam("distance_", distance_);
    nh_private.getParam("launch_position_x", launch_pos_.x());
    nh_private.getParam("launch_position_y", launch_pos_.y());
    nh_private.getParam("launch_position_z", launch_pos_.z());
    nh_private.getParam("v_max", v_max);
    nh_private.getParam("a_max", a_max);
}
void DubinsTrajectory::computePoints() {
    ...
    Vec2 c(centre_x - end_pos_.y(), centre_y + end_pos_.x());
    Vec2 d(centre_x + end_pos_.y(), centre_y - end_pos_.x());

    Line2 ab = Line2::Through(a, b);
    Line2 cd = Line2::Through(c, d);

    float x1 = ab.intersection(cd)[0];
    float y1 = ab.intersection(cd)[1];

    // std::cout << ab.intersection(cd) << std::endl;
    ...
    float cos_ = end_pos_.x() / sqrt((end_pos_.x() * end_pos_.x() + end_pos_.y() * end_pos_.y()));
    float sin_ = end_pos_.y() / sqrt((end_pos_.x() * end_pos_.x() + end_pos_.y() * end_pos_.y()));

    Point first(x1 + up * cos_, y1 + up * sin_, launch_pos_.z(), angle);
    Point third(x1 - up * cos_, y1 - up * sin_, launch_pos_.z(), angle);
    std::cout << "Check2" << std::endl;

    cos_ = -end_pos_.y() / sqrt((end_pos_.x() * end_pos_.x() + end_pos_.y() * end_pos_.y()));
    sin_ = end_pos_.x() / sqrt((end_pos_.x() * end_pos_.x() + end_pos_.y() * end_pos_.y()));
    Point second(centre_x - (radius + 1) * cos_, centre_y - (radius + 1) * sin_, launch_pos_.z(), angle);
...
void DubinsTrajectory::generateTrajectory() {
    ...
    for (int i = 0; i < 4; ++i) {
        std::cout << segment_times_[i] << " ";
    }
    std::cout << std::endl;
    // std::cout << segment_times_[0] << std::endl;
    // std::cout << vertices_[0] << std::endl;
    ...
    std::string frame_id = "default";  // LOOK HERE
